...
public interface Parameter<T extends de.flapdoodle.eval.values.Value<?>> {
	 ...
	 *
	 * @see de.flapdoodle.eval.evaluatables.basic.Conditional for an example.
	 */
	@Value.Default
	...
	default void validate(Token token, de.flapdoodle.eval.values.Value<?> parameterValue) throws EvaluationException {
		Optional<EvaluatableException> error = validationError(parameterValue);
		if (error.isPresent()) {
	...
	@Value.Auxiliary
	default Optional<EvaluatableException> validationError(de.flapdoodle.eval.values.Value<?> parameterValue) {
		if (type().isInstance(parameterValue)) {
			T value = type().cast(parameterValue);
			for (ParameterValidator<T> validator : validators()) {
				Optional<EvaluatableException> error = validator.validate(value);
				if (error.isPresent()) {
					return error;
				}
			}
		} else {
			return Optional.of(EvaluatableException.ofUnsupportedDataTypeInOperation());
		}
