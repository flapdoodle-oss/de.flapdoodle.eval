package de.flapdoodle.eval.evaluatables.validation;
...
public class NumberValidator implements ParameterValidator<Value.NumberValue> {
    ...
    public Optional<EvaluatableException> validate(Value.NumberValue parameterValue) {
        if (!test.test(parameterValue.wrapped())) {
            return Optional.of(new EvaluatableException(errorMessageTemplate.replace("%s", parameterValue.wrapped().toString())));
        }
